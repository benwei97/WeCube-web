...
- Web and mobile automatically remember the last successfully published local meetup area (`users.savedMeetupLocation`) and meetup competitions (`users.savedMeetupCompetitions`). Only fulfillment methods used on the new listing update their defaults; preference-write failures do not fail publishing. Enabling a meetup method on a new listing prefills remembered choices only if no selection exists. Past competitions are excluded, bookmarks stay separate, and existing drafts/listing edits remain independent. There are no extra save/use/clear controls in Sell or Profile.
...
- Users can ignore the prompt if they do not want to leave a review
...
- Uploads and deletes go through Firebase Cloud Functions using short-lived signed S3 URLs
